# ADK 2.0 Migration Plan

This document captures every code change required to align the
`oracledb-vertexai-demo` with the **SQLspec ADK 2.0 clean break**. The
plan assumes a destructive cutover. SQLspec 2.0 ships a first-party
cutover migration (`0002_reset_adk_tables.py`) that drops the legacy
ADK tables and creates the new 2.0 schema in one transaction; the demo
runs it via the existing `make migrate` target. No demo-side migration
...
```diff
- "sqlspec[adk,mypyc,oracledb,performance]>=0.46.3",
+ "sqlspec[adk,mypyc,oracledb,performance]>=0.48.0",
- "google-adk>=2.0.0b1",
+ "google-adk>=2.0.0",
```
...
> Why: SQLspec 0.48+ introduces the atomic `append_event_and_update_state`
> kwargs (now `(event_record, app_name, user_id, session_id, state, *,
> app_state=None, user_state=None)`), the `EMBEDDING_PRESETS` registry +
> `resolve_embedding_config` helper, the BigQuery analytics-replica
> store, the new clean-break grep guards, the `event_json` →
> `event_data` column rename, the three scoped-state tables
...
> the new schema via `make migrate`. ADK 2.0 stable removes the
> `2.0.0b1` API rough edges.
...
ADK 2.0 ships an `EMBEDDING_PRESETS` registry plus a
`resolve_embedding_config(memory_config)` helper that raises
`ImproperConfigurationError` when neither `embedding_dimension` nor
`embedding_preset` is supplied. **The helper is not auto-called by the
store today** — the memory store accepts any extension_config and never
validates the embedding section at `__init__`. To get fail-fast
behavior, wire the call into demo startup (e.g. inside `create_config`
or an `on_startup` hook):

```python
from sqlspec.extensions.adk.memory import resolve_embedding_config

resolve_embedding_config(adk_config.get("memory"))
```

Adopt the preset key as a forward-compatible signal: the moment a future
sqlspec release flips this to an init-time check (or a per-driver
vector-search path enforces it), the demo is already configured.

The demo uses `gemini-embedding-001` at **3072-dim** for product vectors
(native, configured via `EMBEDDING_DIMENSIONS = 3072`). The ADK memory
store should use a **separate model dedicated to recall** —
`gemini-embedding-002` at 1536-dim is the right choice today: smaller
storage footprint, current-generation Vertex AI model, MRL-friendly.
...
+ ADK_EMBEDDING_PRESET: str = "gemini-embedding-002"   # 1536-dim, normalized
...
"adk": {
    ...
    "in_memory": True,
    "memory": {
        "embedding_preset": self.ADK_EMBEDDING_PRESET,   # required in 2.0
    },
}
```

Available presets (from `sqlspec.extensions.adk.memory.presets.EMBEDDING_PRESETS`):
`gemini-embedding-002` (1536), `gemini-embedding-001` (768),
`embeddinggemma-300m` (768), `text-embedding-005` (768),
`text-embedding-004` (768), `text-embedding-3-large` (3072),
`text-embedding-3-small` (1536), `text-embedding-ada-002` (1536).
Register custom models at runtime with
`register_embedding_preset(name, EmbeddingPreset(...))`.
...
APIs over `adk_memory_entries`. The session store and memory store
share the same Oracle config (and pool) so there's no extra connection
cost.

### 2c. Embed text for memory writes (Vertex AI helper) — optional today

**Important:** The Oracle ADK memory store searches **text only** today.
`OracleAsyncADKMemoryStore.search_entries` dispatches to either an Oracle
Text `CONTAINS(content_text, …)` path or a `LOWER(content_text) LIKE …`
fallback — there is **no vector-search path** on the Oracle memory store
in sqlspec 2.0. Anything you put in `content_json["embedding"]` is
stored but **not queried** for retrieval.

You have two reasonable choices:

1. **Skip the embedding helper entirely.** Lexical recall via Oracle Text
   on `content_text` is sufficient for the "the usual?" demo. Drop §2c
   and pass an empty/no `embedding` key in `content_json`. The
   `embedding_preset` setting still has value as forward-compatibility
   signal (see §2a).
2. **Keep the embedding helper** anticipating a future sqlspec vector
   path. The plumbing below pre-stores the vector so the day vector
   search lands you can flip a flag without re-embedding history. This
   is what the rest of §2 assumes.

The demo already has Vertex AI plumbing (`FlashLiteIntentClassifier`,
product vector service); reuse it if you pick option 2.

Add a small embedding helper service if one doesn't exist:

```python
# src/app/domain/chat/services/_adk_memory_embedding.py  (NEW)
"""Embedding helper for ADK memory entries.

Uses gemini-embedding-002 at 1536-dim to match ADKMemoryConfig.
Separate from the product-side embedding (gemini-embedding-001 @ 3072).
"""
...
_MEMORY_EMBEDDING_MODEL = "gemini-embedding-002"
_MEMORY_EMBEDDING_DIM = 1536


async def embed_memory_text(client: "genai.Client", text: str) -> "list[float]":
    """Embed a single memory text into a normalized 1536-dim vector."""
    response = await client.aio.models.embed_content(
        model=_MEMORY_EMBEDDING_MODEL,
        contents=text,
        config={"output_dimensionality": _MEMORY_EMBEDDING_DIM, "task_type": "RETRIEVAL_DOCUMENT"},
    )
    return list(response.embeddings[0].values)


async def embed_memory_query(client: "genai.Client", query: str) -> "list[float]":
    """Embed a query text (RETRIEVAL_QUERY task_type) — different from the document path."""
    response = await client.aio.models.embed_content(
        model=_MEMORY_EMBEDDING_MODEL,
        contents=query,
        config={"output_dimensionality": _MEMORY_EMBEDDING_DIM, "task_type": "RETRIEVAL_QUERY"},
    )
    return list(response.embeddings[0].values)
```

### 2d. Write a memory entry when an order completes
...
async def remember_order(
    ...
    *,
    ...
    embedded = await embed_memory_text(genai_client, summary)
    ...
    record: MemoryRecord = {
        ...
        "author": "system",
        "timestamp": now,
        "content_json": {"parts": [{"text": summary}], "embedding": embedded},
...
intent. Wire the new `memory_store` and `genai_client` dependencies via
the existing Dishka container.

### 2e. Recall past orders at the start of a turn
...
### 2f. Retention housekeeping (optional but recommended for a demo)
...
def adk_memory_prune(days: int) -> None:
    ...
    import anyio
    from sqlspec.adapters.oracledb.adk import OracleAsyncADKMemoryStore
    from app.config import oracle_async_config

    async def _run() -> None:
        store = OracleAsyncADKMemoryStore(oracle_async_config)
...
ADK 2.0 keeps `update_session_state` working — this code is **not
broken** — but the 2.0-idiomatic pattern is to fold the durable state
mutation into the event that caused it. That guarantees the event insert,
session UPDATE, and (when used) `app_state` / `user_state` UPSERTs land
...
Why: `SQLSpecSessionService.append_event` (2.0) calls
...
Per the SQLspec 2.0 service contract, `SQLSpecSessionService.store` is a
real attribute (not optional) and every retained adapter implements
`update_session_state`. **But the signature changed in 2.0** — it now
