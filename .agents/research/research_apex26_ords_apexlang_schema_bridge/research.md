...
- SQLcl 26.1.2 is the important APEXlang tooling floor. Its APEX commands support generate, export, validate, and import workflows. Official APEXlang source structure uses `shared_components` and `supporting_objects` with underscores, so earlier hyphenated local plans should be corrected.
...
- Official project structure uses:
  - `application.apx`
  - `pages/`
  - `shared_components/`
  - `supporting_objects/`
...
- Put source under `src/apex/<app-alias>/` and use the official underscore directory names.
