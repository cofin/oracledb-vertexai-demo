...
3. **JSON I/O goes through `app.utils.serialization`.** Use `from_json` / `to_json` (re-exports of `sqlspec.utils.serializers`, mirroring `dma/accelerator/src/py/dma/utils/serialization.py`). Direct `json.loads` / `json.dumps` is banned for any reader/writer added or touched in Phase 5; the wrapper plays nicely with msgspec / orjson and project-wide type encoders.
...
- [x] **1.8** **CLI restructure — `coffee` becomes hand-rolled.** Mirrors `dma/accelerator/src/py/dma/cli/main.py` and `dma/accelerator/src/py/dma/cli/commands/server.py`. [6488db1]
