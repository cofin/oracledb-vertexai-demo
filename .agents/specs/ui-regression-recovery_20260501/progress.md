...
- Follow-up sync 2026-05-01: Alpine has been removed from the current UI stack. Chat intent chips now show the effective intent from actual tool telemetry, so a breakfast/menu lookup that ran product vector search displays as `PRODUCT_RAG` even if the initial classifier returned general conversation. Chat-side vector lookups now record metrics, and the EXPLAIN PLAN table wraps in place instead of forcing horizontal scroll on normal desktop widths.
