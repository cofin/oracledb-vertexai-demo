...
4. **Frontend = HTMX + Tailwind v4 + vanilla Vite modules + ApexCharts via litestar-vite mode=template.** React/TanStack deleted entirely, and Alpine.js is no longer part of the active frontend stack. Visual polish must match or exceed the React version on the explore page (which combines old `performance` + `vector-demo` into one page with 5 panels including a live Oracle EXPLAIN PLAN viewer).
...
6. **Cross-chapter file contracts must be pinned.** Historical note: early Ch 3/Ch 4 planning used `dist/classify-compare.json` as a shared artifact. That contract was later superseded by `ui-regression-recovery_20260501`, which removed the classify-compare UI/API surface. New specs should not restore that file contract unless a new flow explicitly reopens the feature.
...
11. **CLI ownership is split:** `coffee` is the hand-rolled app CLI (`run`, `upgrade`, `load-fixtures`, `clear-cache`, `model-info`, `bulk-embed`, `export-fixtures`). `coffee upgrade` is the packaged/end-user install command; raw SQLSpec developer commands, assets, and infrastructure stay on `python manage.py ...`. New plans should not route through `uv run app`, `coffee assets`, `coffee downgrade`, `coffee classify-compare`, or `coffee_demo_group`.
