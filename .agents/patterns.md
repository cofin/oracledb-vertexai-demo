...
- **CSRF for HTMX:** include `<meta name="csrf-token" content="{{ request.scope['csrf_token'] }}">` in `base.html.j2`. Wire HTMX to forward it via `htmx:configRequest` or use `registerHtmxExtension()` to populate the `X-CSRFToken` header on every non-GET request. GET-only endpoints (charts, EXPLAIN PLAN, classify-compare) skip the token plumbing.
...
- **CLI split — `coffee` vs `manage.py`:** `coffee` exposes only production-app commands (`run` / `bulk-embed` / `clear-cache` / `model-info` / `load-fixtures` / `export-fixtures`) via a hand-rolled `rich_click` group. **Do not call `litestar_group()` from `coffee`** — that drags in litestar-builtin `info` / `routes` / `schema` / `sessions` AND materializes `app.config.db` even on `--help`. Migrations + assets + infra all live exclusively on `manage.py` (`database upgrade`, `assets build`, `infra start-local-container`). Mirrors `dma/accelerator`'s `dma`-vs-`manage.py` split. Architecture-enforced via `test_cli_surface.py` — including `test_coffee_help_does_not_construct_db_config` which poison-checks the import path on `coffee --help`.
- **Module-scoped path constants for testable file lookups:** when a controller reads from a contract file (e.g. `dist/classify-compare.json`), expose the path as a module-level constant (`CLASSIFY_COMPARE_PATH = ...`). Tests monkey-patch the constant to point at a tmp file. Cleaner than a class attribute, settings entry, or environment variable because the artifact lives with the controller that owns it.
- **Toolchain invariants belong in tests, not docs:** lock pyproject/Makefile/.gitignore/Dockerfile contracts via `test_repo_layout_invariants.py` so a future commit can't silently regress (re-introduce `bun`, `litestar-htmx`, `coffee upgrade`, `src/py`, etc.). 14 such invariants now guard Ch 4's toolchain finalize.
