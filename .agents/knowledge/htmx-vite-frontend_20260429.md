...
Ch 4 deleted the React + TanStack Router + Bun + Biome frontend wholesale and rebuilt it on HTMX 2.0.10 + Tailwind v4 + Vite-bundled JavaScript/ApexCharts via `litestar-vite` mode=`template` + `litestar.plugins.htmx`. In the same chapter, the source tree was flattened (`src/py/{app,tests}` -> `src/{app,tests}`, `src/js/` deleted entirely). The active Vite/npm root is now `src/resources/`, where `vite.config.ts`, `package.json`, `package-lock.json`, `tsconfig.json`, source modules, generated bridge files, public assets, and `node_modules/` live. The `coffee` CLI was restructured: it became a hand-rolled `rich_click` group exposing only production-app commands (`run`, `upgrade`, `bulk-embed`, `clear-cache`, `model-info`, `load-fixtures`, `export-fixtures`). `coffee upgrade` is the packaged/end-user install path; raw SQLSpec developer commands, assets, and infra live on `manage.py`. The chapter delivered two server-rendered pages — chat (feature parity with the deleted React) and explore (vector search, EXPLAIN PLAN viewer, metrics summary, latency time-series, and vector storage calculator).
...
- With `src/resources/` as the Vite project root, `vite.config.ts` uses `publicDir: "public"` and entry keys such as `main.js` / `styles.css`; Jinja `vite(...)` calls must use those same keys.
...
- `src/resources/{vite.config.ts,package.json,package-lock.json,tsconfig.json}` — Vite/npm project root
...
- Brand assets at `src/js/public/` had to be rescued before `git rm -rf src/js/`. They moved to `src/resources/public/`. After `src/resources/` became the Vite root, `publicDir` is `public`.
...
- Vite bundle dir is co-located with the web domain peer-package (`src/app/domain/web/static/`) so templates and assets stay together. The path appears in `src/resources/vite.config.ts`, `ViteConfig.paths.bundle_dir`, and `.gitignore` — they must agree.
...
- `src/resources/` is the Vite project root, so `publicDir` is `public`; if the root changes again, remember that Vite resolves `publicDir` from the Vite root, not from Litestar's Python package.
...
- `make install` also runs `manage.py assets build` after `manage.py assets install`, so a fresh checkout has a production manifest even when `VITE_DEV_MODE` stays false or is toggled later.
