...
SQL_DIR = PY_ROOT / "app" / "db" / "sql"
DOMAIN_DIR = PY_ROOT / "app" / "domain"
...
@pytest.mark.parametrize("service_file", SERVICE_FILES, ids=lambda p: p.relative_to(PY_ROOT).as_posix())
def test_no_inline_sql_strings_in_domain_services(service_file: Path) -> None:
    text = service_file.read_text()
    matches = INLINE_SQL_PATTERN.findall(text)
    assert not matches, (
        f"{service_file.relative_to(PY_ROOT)} still contains inline SQL keywords: "
