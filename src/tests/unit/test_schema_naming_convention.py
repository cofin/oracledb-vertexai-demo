...
@pytest.mark.parametrize(
    "schema_file",
    _schema_files(),
    ids=lambda p: p.relative_to(PY_ROOT).as_posix(),
)
def test_no_banned_suffixes_in_struct_names(schema_file: Path) -> None:
    ...
    assert not offenders, (
        f"{schema_file.relative_to(PY_ROOT)} declares Structs with banned suffixes: "
...
def test_dto_wording_absent_from_schema_docstrings() -> None:
    ...
    for path in _schema_files():
        ...
        for match in pattern.finditer(text):
            line_no = text.count("\n", 0, match.start()) + 1
            offenders.append((path.relative_to(PY_ROOT).as_posix(), f"line {line_no}"))
