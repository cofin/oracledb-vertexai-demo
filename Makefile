...
upgrade: setup-env ## Upgrade all dependencies to latest stable versions
	...
	@uv run python -c "import re; from datetime import datetime, timezone; date_str = datetime.now(timezone.utc).strftime('%Y-%m-%d'); dt_str = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'); text = open('pyproject.toml').read(); text = re.sub(r'package_date = \".*?\"', f'package_date = \"{date_str}\"', text); open('pyproject.toml', 'w').write(text); text = open('uv.toml').read(); text = re.sub(r'exclude-newer = \".*?\"', f'exclude-newer = \"{dt_str}\"', text); open('uv.toml', 'w').write(text);"
