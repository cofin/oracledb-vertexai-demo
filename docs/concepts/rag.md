...
Coffee solves that the same way most production RAG systems do: retrieve
real menu rows from Oracle, hand them to the model as the only acceptable
source, and refuse to invent products that aren't on the list.
...
*not* free-form prose. The agent's tool returns this list to the model as
its tool output.
...
fast classifier reads the user's turn first and routes it to a
deterministic handler. For `PRODUCT_RAG`, the handler runs the vector
search itself and formats the answer from the returned rows — the LLM
never gets to draft a speculative reply. That has three consequences:
...
3. `search_products_by_vector` is *also* registered as a tool on the
   ...
   — and even then, if the model's reply looks like product talk, the
   runner re-grounds it from a real vector search before emitting the
   final event.
...
- **The model skips the tool**: the runner's grounding fallback re-runs
  product search and formats the answer from the returned rows rather than
  trusting the model's tool-less draft.
- **The model emits a tool-schema artifact instead of an answer**: same
  fallback. Better an honest grounded answer than a confused one.
...
- **The walkthrough** walks through one full RAG turn: see [the
  walkthrough](../tour.md).
- **The agent flow** explains how parallel intent classification *and* the
  agent's own tool reasoning route between RAG and general conversation:
  see [Google ADK](agent-flow.md).
