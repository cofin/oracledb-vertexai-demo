...
async def _setup_test_db(oracle_connection: oracledb.AsyncConnection) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        await cursor.execute("DELETE FROM inventory")
        await cursor.execute("DELETE FROM product") 
        await cursor.execute("DELETE FROM shop")
        await cursor.execute("DELETE FROM company")
