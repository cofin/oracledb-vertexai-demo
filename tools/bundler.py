...
SITE_PACKAGES_TREE_DEPTH = 4

# Mapping from Rust target to Python Build Standalone URL (Python 3.13.13).
# Source: build.rs in pyapp 0.29.0 (pyapp is pinned to 20251014 / 3.13.9;
# these overrides pull the newer stripped install-only artifacts).
# Release: https://github.com/astral-sh/python-build-standalone/releases/tag/20260414
...
class BuildConfig:
    """Resolved inputs for a bundled Python distribution."""
...
def find_site_packages(python_root: Path, target: str, python_version: str) -> Path:
    ...
    for root, _, _ in os.walk(python_root):
        ...
        if depth < SITE_PACKAGES_TREE_DEPTH:
...
def report_package_sizes(site_packages: Path) -> None:
    """Print a sorted list of the largest packages in site-packages."""

    pkg_sizes = []
    for item in site_packages.iterdir():
        if item.is_dir() and not item.name.startswith("__") and not item.name.endswith(".dist-info"):
            size = sum(f.stat().st_size for f in item.rglob("*") if f.is_file())
            pkg_sizes.append((item.name, size))

    pkg_sizes.sort(key=operator.itemgetter(1), reverse=True)

    console.print()
    left_aligned_rule("Package Sizes", style="#34A853")
    console.print("[bold]Top 20 Production Packages by Size:[/]")
    for name, size in pkg_sizes[:20]:
        console.print(f"  {name}: {size / (1024 * 1024):.2f} MB")
    console.print()
...
def render_install_dir_expression(install_dir: Path) -> str:
    ...
    with contextlib.suppress(ValueError):
        relative = install_dir.relative_to(config_dir)
        expression = 'directories::BaseDirs::new().expect("could not find base directories").config_dir()'
...
@click.option("--project-dir", type=click.Path(path_type=Path), help="Project directory")  # type: ignore[type-var]
...
def resolve_target(target: str | None) -> str:
    """Resolve the Rust target from CLI input, environment, or host platform."""
...
def resolve_build_config(
    ...
    """Resolve CLI options into concrete bundler paths and target metadata."""
    ...
    return BuildConfig(
        ...
        python_version=python_version or (infer_python_version(url) if url else DEFAULT_PYTHON_VERSION),
        output=resolve_output_path(output, output_name, resolved_project, resolved_target),
    )


def summarize_build_config(config: BuildConfig, python_archive: Path | None) -> None:
    """Print resolved build paths and target metadata."""
    ...
    if config.python_url:
        console.print(f"  Source URL:   {config.python_url}")
    if python_archive:
        console.print(f"  Source file:  {python_archive}")
    console.print()


def prepare_work_dir(work_dir: Path | None) -> Path:
    """Prepare or create the temporary work directory."""

    if not work_dir:
        return Path(tempfile.mkdtemp(prefix="bundler-"))
...
def resolve_python_archive(config: BuildConfig, python_archive: Path | None, refresh: bool) -> Path:
    """Resolve the Python build archive, downloading it into cache when needed."""

    if python_archive:
        return python_archive.expanduser().resolve()

    assert config.python_url is not None  # noqa: S101 - Validated before archive resolution
    cache_root = config.cache_dir / "python-build-standalone" / config.target
    ensure_directory(cache_root)
    archive_path = cache_root / Path(config.python_url).name
    if refresh and archive_path.exists():
        archive_path.unlink()
    if not archive_path.exists():
        download_with_retry(config.python_url, archive_path)
    return archive_path


def repackage_python(python_root: Path, archive_path: Path, output: Path) -> None:
    """Repackage the prepared Python root into the requested archive format."""

    console.print(f"[blue]Repackaging[/] to {output}...")
...
def create_bundle(
    ...
    """Build the bundled Python distribution in the prepared work directory."""

    if pyapp_dir:
        patch_pyapp_install_dir(pyapp_dir.expanduser().resolve(), config.install_dir)
    else:
        console.print("[dim]Note:[/] pass --pyapp-dir to patch PyApp for the install directory.")

    archive_path = resolve_python_archive(config, python_archive, refresh)
    extract_dir = work_dir_path / "extracted"
    extract_dir.mkdir(parents=True, exist_ok=True)
    extract_archive(archive_path, extract_dir)

    python_root = resolve_python_root(extract_dir)
    site_packages = find_site_packages(python_root, config.target, config.python_version)
    ...
    report_package_sizes(site_packages)
    repackage_python(python_root, archive_path, config.output)

    output_size = config.output.stat().st_size / (1024 * 1024)
    console.print()
    console.print(f"[bold green]Success![/] Created {config.output} ({output_size:.1f} MB)")
...
@click.option("--requirements", type=click.Path(path_type=Path), help="Path to requirements.txt")  # type: ignore[type-var]
...
@click.option("--work-dir", type=click.Path(path_type=Path), help="Working directory for temporary files")  # type: ignore[type-var]
...
@click.option("--output-name", help="Override output filename (stored under dist/)")
@click.option("--pyapp-dir", type=click.Path(path_type=Path), help="Path to a PyApp checkout to patch install dir")  # type: ignore[type-var]
def build_bundle(
    target: str | None,
    requirements: Path | None,
    output: str | None,
    ...
    summarize_build_config(config, python_archive)

    work_dir_path = prepare_work_dir(work_dir)
    try:
        create_bundle(
            config=config,
            python_archive=python_archive,
