...
-- configure-apex-cdn.sql
-- Updates Oracle APEX to use Oracle's public CDN for static assets instead of serving them locally.
-- This keeps the ORDS deployment stateless on Cloud Run.
...
DECLARE
    ...
    v_target_prefix CONSTANT VARCHAR2(255) := 'https://static.oracle.com/cdn/apex/26.1.0/';
BEGIN
    ...
    -- 3. Update parameter if not already pointing to target CDN
    IF v_current_prefix IS NULL OR v_current_prefix != v_target_prefix THEN
        ...
        COMMIT;
        dbms_output.put_line('APEX IMAGE_PREFIX successfully updated to CDN: ' || v_target_prefix);
    ELSE
        dbms_output.put_line('APEX IMAGE_PREFIX is already set to CDN. No action required.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        dbms_output.put_line('Error updating APEX IMAGE_PREFIX: ' || SQLERRM);
        RAISE;
END;
/
