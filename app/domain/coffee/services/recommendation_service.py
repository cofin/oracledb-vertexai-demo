...
from app.db.models import Shop, Inventory
...
from app.domain.coffee.schemas import CoffeeChatReply, ChatMessage, PointsOfInterest

logger = structlog.get_logger()


class NativeRecommendationService:
    ...
    async def get_recommendation(self, query: str, session_id: str = None) -> CoffeeChatReply:
    ...
    async def _route_locations_question(
        self,
        query: str,
        matched_product_ids: Sequence[int] = None,
        chat_metadata: dict[str, Any] = None,
        ...
        if any(word in query_lower for word in location_keywords) and matched_product_ids:
            ...
            for shop in shops_with_products:
                locations.append(PointsOfInterest(
                    id=shop.id,
                    name=shop.name,
    ...
    async def stream_recommendation(self, query: str, session_id: str = None):
